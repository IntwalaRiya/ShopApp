...
function MainNavbar() {
  ...
  let cart;
  if(auth.isCustomer && !(auth.isAdmin || auth.isSeller || auth.isShipper)){
    cart = (
      <Link to='/cart'>
        <i class='fa fa-shopping-cart cart-icon' aria-hidden='true'>Cart</i>
      </Link>
    );
  }
  /* Dashboard link */
  ...
  if (loading) {
    ...
    userDropDown = (
      <span className='nav-link'>
        <NavDropdown title={user.Firstname.S} id='basic-nav-dropdown'>
          <NavDropdown.Item>
  ...
  return (
    <Navbar bg='light' expand='xxl' className='main-navbar' expanded={expanded}>
      ...
      <Navbar.Collapse id='basic-navbar-nav'>
        <Nav>
          {user && <div className='welcome'>Hello, {user.Firstname.S}</div>}
