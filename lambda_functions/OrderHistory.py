...
def lambda_handler(event, context):
    try:
        ...
        cart = dynamodb.scan(
            ...
            FilterExpression='#N = :user AND #Q.Pending = :statevalue',  # Replace 'ColumnName' with your actual attribute name
            ExpressionAttributeValues={':user': {'S': event['User']},':statevalue': {'BOOL': False}}  # Assuming 'S' for string, adjust for other types
        )
        print(cart)
        if 'Items' in cart:
            order_dir = {}
            ...
            return {
                ...
                'orders': {},
                'message': 'No orders history found'
            }
    except:
        return {
            'statusCode': 500,
